...
template <Dimension D> void Lattice<D>::Render(RenderContext<D> *p_Context) const
{
    ...
    if constexpr (D == D2)
        ...
        case Shapes2::Triangle:
            Run([p_Context](const f32v2 &p_Pos, const Lattice<D2> &p_Lattice) {
    ...
    else
    {
        ...
        case Shapes3::Triangle:
            Run([p_Context](const f32v3 &p_Pos, const Lattice<D3> &p_Lattice) {
                p_Context->SetTranslation(p_Pos);
                p_Context->Triangle(f32v2{p_Lattice.ShapeSize});
            });
            return;
        case Shapes3::Square:
            Run([p_Context](const f32v3 &p_Pos, const Lattice<D3> &p_Lattice) {
                p_Context->SetTranslation(p_Pos);
                p_Context->Square(f32v2{p_Lattice.ShapeSize});
            });
            return;
        case Shapes3::NGon:
            Run([p_Context](const f32v3 &p_Pos, const Lattice<D3> &p_Lattice) {
                p_Context->SetTranslation(p_Pos);
                p_Context->NGon(p_Lattice.NGonSides, f32v2{p_Lattice.ShapeSize});
            });
            return;
        ...
        case Shapes3::RoundedSquare:
            Run([p_Context](const f32v3 &p_Pos, const Lattice<D3> &p_Lattice) {
                p_Context->SetTranslation(p_Pos);
                p_Context->RoundedSquare(f32v2{p_Lattice.ShapeSize}, p_Lattice.Diameter);
            });
            return;
        case Shapes3::Mesh:
            Run([p_Context](const f32v3 &p_Pos, const Lattice<D3> &p_Lattice) {
