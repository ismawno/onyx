                                  ...
                                  const f32 p_Rotation) noexcept
{
    ...
    const mat2 linear = Trigonometry<2>(p_Rotation).RotationMatrix() * scale;
    const vec2 translation = p_Position - linear * p_Origin;

    return mat4{vec4(linear[0], 0.f, 0.f), vec4(linear[1], 0.f, 0.f), vec4(0.f, 0.f, 1.f, 0.f),
                                         ...
                                         const f32 p_Rotation) noexcept
{
    ...
    const mat2 linear = scale * Trigonometry<2>(-p_Rotation).RotationMatrix();
    const vec2 translation = p_Origin - linear * p_Position;
                                  ...
                                  const quat &p_Rotation) noexcept
{
    ...
    const mat3 linear = glm::toMat3(p_Rotation) * scale;
    const vec3 translation = p_Position - linear * p_Origin;

    return mat4{vec4(linear[0], 0.f), vec4(linear[1], 0.f), vec4(linear[2], 0.f), vec4(translation, 1.f)};
}
                                         ...
                                         const quat &p_Rotation) noexcept
{
    ...
    const mat3 linear = scale * glm::toMat3(glm::conjugate(p_Rotation));
    const vec3 translation = p_Origin - linear * p_Position;
...
ONYX_DIMENSION_TEMPLATE void ITransform<N>::UpdateComponents() noexcept
{
    ...
    if constexpr (N == 2)
    {
        m_Scale = vec2{glm::length(vec2{m_LocalTransform[0]}), glm::length(vec2{m_LocalTransform[1]})};

        // No need to normalize: scale cancels out
        m_Rotation = glm::atan(m_LocalTransform[0].y, m_LocalTransform[0].x);
        m_Position = vec2{m_LocalTransform[3]};
    }
}
