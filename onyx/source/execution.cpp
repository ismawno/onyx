...
static VKit::CommandPool createCommandPool(const u32 family)
{
    ...
    return ONYX_CHECK_VKIT_RESULT(VKit::CommandPool::Create(device, family, VK_COMMAND_POOL_CREATE_TRANSIENT_BIT));
}
...
static void createTransientCommandPools()
{
    ...
    *s_Graphics = createCommandPool(gindex);
    ...
    {
        ONYX_CHECK_VKIT_RESULT(s_Graphics->SetName("onyx-transient-graphics-command-pool"));
        if (gindex != tindex)
        {
            ONYX_CHECK_VKIT_RESULT(s_Transfer->SetName("onyx-transient-transfer-command-pool"));
        }
    }
}
...
CommandPool *FindSuitableCommandPool(const u32 family)
{
    for (CommandPool &pool : *s_CommandPools)
        ...
        {
            ONYX_CHECK_VKIT_RESULT(pool.Pool.Reset());
            pool.NextCommand = 0;
            return &pool;
        }
...
VkCommandBuffer Allocate(CommandPool *pool)
{
    ...
    pool->CommandBuffers.Append(ONYX_CHECK_VKIT_RESULT(pool->Pool.Allocate()));
    ++pool->NextCommand;
...
void BeginCommandBuffer(const VkCommandBuffer commandBuffer)
{
    ...
    ONYX_CHECK_VKIT_RESULT(table->BeginCommandBuffer(commandBuffer, &beginInfo));
}
void EndCommandBuffer(const VkCommandBuffer commandBuffer)
{
    const auto table = GetDeviceTable();
    ONYX_CHECK_VKIT_RESULT(table->EndCommandBuffer(commandBuffer));
}
...
void Initialize(const Specs &specs)
{
    ...
    for (u32 i = 0; i < queues.GetSize(); ++i)
    {
        ...
        ONYX_CHECK_VKIT_RESULT(table->CreateSemaphore(device, &info, nullptr, &semaphore));
