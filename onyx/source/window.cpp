...
void Window::createSyncData()
{
    ...
    for (u32 i = 0; i < imageCount; ++i)
    {
        ...
        WindowSyncData *sync = m_SyncData.Append(tier->Create<WindowSyncData>());
        ONYX_CHECK_VKIT_RESULT(table->CreateSemaphore(device, &semaphoreInfo, nullptr, &sync->ImageAvailableSemaphore));
        ONYX_CHECK_VKIT_RESULT(table->CreateSemaphore(device, &semaphoreInfo, nullptr, &sync->RenderFinishedSemaphore));
    }
}
...
Window::~Window()
{
    // ONYX_CHECK_VKIT_RESULT(DeviceWaitIdle());
    drainWork();
    destroySyncData();
...
static VkPresentModeKHR asVulkanPresentMode(const PresentMode mode)
{
    switch (mode)
    {
...
VkSemaphore Window::GetImageAvailableSemaphore() const
{
    return m_SyncData[m_SyncIndex]->ImageAvailableSemaphore;
}
VkSemaphore Window::GetRenderFinishedSemaphore() const
{
