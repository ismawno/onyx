...
using ResourceFlags = u8;
enum ResourceFlagBit : ResourceFlags
{
    ResourceFlag_Locked = 1 << 0,
};
...
ResourceFlags s_ResourceFlags = 0;
SyncFlags s_SyncFlags = 0;
...
{
    const u32 bid = s_Buffers->Resources.Insert(ONYX_CHECK_EXPRESSION(builder.Build()));
...
{
    const u32 sid = samplers.Insert(ONYX_CHECK_EXPRESSION(builder.Build()));
...
static Resource createSampler(TKit::ArenaHive<VKit::Sampler> &samplers, const SamplerData &data)
{
    return createSampler(samplers, VKit::Sampler::Builder(GetDevice())
                                       .SetMipmapMode(toVulkan(data.Mode))
                                       .SetMinFilter(toVulkan(data.MinFilter))
                                       .SetMagFilter(toVulkan(data.MagFilter))
                                       .SetAddressModeU(toVulkan(data.WrapU))
                                       .SetAddressModeV(toVulkan(data.WrapV))
                                       .SetAddressModeW(toVulkan(data.WrapW)));
}
...
static void bindSampler(const Resource handle)
{
    ...
    {
        ONYX_CHECK_EXPRESSION(sampler.SetName(TKit::Format("onyx-resources-sampler-{:#010x}", handle).c_str()));
    }
...
Resource CreateImage(const VKit::DeviceImage::Builder &builder)
{
    ...
    img.Image = ONYX_CHECK_EXPRESSION(builder.Build());
...
Resource CreateImage(const ImageData &data)
{
    const Resource handle = CreateImage(VKit::DeviceImage::Builder(
        GetDevice(), GetVulkanAllocator(), VkExtent2D{data.Width, data.Height}, data.Format,
    ...
    VKit::DeviceBuffer uploadBuffer =
        ONYX_CHECK_EXPRESSION(VKit::DeviceBuffer::Builder(GetDevice(), GetVulkanAllocator(),
    ...
    if (IsDebugUtilsEnabled())
    {
        ...
        ONYX_CHECK_EXPRESSION(img.Image.SetName(name.c_str()));
...
Resource CreateTexture(const Resource handle, const u32 viewIndex)
{
    ...
    const VkImageView view =
        viewIndex == TKIT_U32_MAX ? ONYX_CHECK_EXPRESSION(img.AddImageView()) : img.GetView(viewIndex);
...
void Sync(const SyncFlags flags)
{
    ...
    TKIT_ASSERT(!(s_ResourceFlags & ResourceFlag_Locked),
                "[ONYX][RESOURCES] Cannot upload/mutate resource data because it is locked, either by the "
...
bool RequestSync(const SyncFlags flags)
{
    if (s_ResourceFlags & ResourceFlag_Locked)
    {
        s_SyncFlags |= flags;
        return false;
    }
...
void LockSync()
{
    s_ResourceFlags |= ResourceFlag_Locked;
}
void UnlockSync()
{
    s_ResourceFlags &= ~ResourceFlag_Locked;
    if (s_SyncFlags)
        return Sync(s_SyncFlags);
}
bool IsSyncLocked()
{
    return s_ResourceFlags & ResourceFlag_Locked;
}
