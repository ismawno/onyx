...
static TKit::StaticArray8<VkSemaphore> createRenderFinishedSemaphores(const u32 p_Count) noexcept
{
    ...
    for (VkSemaphore &sp : semaphores)
    {
        ...
        TKIT_ASSERT_RETURNS(vkCreateSemaphore(Core::GetDevice(), &semaphoreInfo, nullptr, &sp), VK_SUCCESS,
...
static void destroyRenderFinishedSemaphores(const TKit::Span<VkSemaphore> p_Semaphores) noexcept
{
    for (const VkSemaphore sp : p_Semaphores)
        vkDestroySemaphore(Core::GetDevice(), sp, nullptr);
}
FrameScheduler::FrameScheduler(Window &p_Window) noexcept
{
    ...
    m_RenderFinishedSemaphores = createRenderFinishedSemaphores(m_SwapChain.GetInfo().ImageData.size());
...
void FrameScheduler::recreateSwapChain(Window &p_Window) noexcept
{
    ...
    const TKit::StaticArray8<VkImageView> imageViews = getIntermediateAttachmentImageViews();
...
TKit::StaticArray8<VkImageView> FrameScheduler::getIntermediateAttachmentImageViews() const noexcept
{
