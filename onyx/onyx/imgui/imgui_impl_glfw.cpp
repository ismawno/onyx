...
// creation, etc.) (Requires: GLFW 3.0+. Prefer GLFW 3.3+/3.4+ for full feature support.)
...
//  (ImGuiBackendFlags_HasMouseCursors) with GLFW 3.1+. Resizing cursors requires GLFW 3.4+! Disable with
//  'io.ConfigFlags |= ImGuiConfigFlags_NoMouseCursorChange'. [X] Platform: Multi-viewport support (multiple windows).
//  Enable with 'io.ConfigFlags |= ImGuiConfigFlags_ViewportsEnable'. [X] Multiple Dear ImGui contexts support.
// Missing features or Issues:
//  [ ] Platform: Touch events are only correctly identified as Touch on Windows. This create issues with some
//  interactions. GLFW doesn't provide a way to identify touch inputs from mouse inputs, we cannot call
//  io.AddMouseSourceEvent() to identify the source. We provide a Windows-specific workaround. [ ] Platform: Missing
//  ImGuiMouseCursor_Wait and ImGuiMouseCursor_Progress cursors. [ ] Platform: Multi-viewport: Missing
//  ImGuiBackendFlags_HasParentViewport support. The viewport->ParentViewportID field is ignored, and therefore
//  io.ConfigViewportsNoDefaultParent has no effect either.
...
//  X11. (#8920, #8921) 2025-09-10: [Docking] Improve multi-viewport behavior in tiling WMs on X11 via the
//  ImGui_ImplGlfw_SetWindowFloating() function. Note: using GLFW backend on Linux/BSD etc. requires linking with -lX11.
//  (#8884, #8474, #8289) 2025-07-08: Made ImGui_ImplGlfw_GetContentScaleForWindow(),
